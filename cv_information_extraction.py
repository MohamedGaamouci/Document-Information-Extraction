...
class cv_information_extraction:
    ...
    def detect_and_ocr_batch(self, images, selected_model=None):
        """
        ...
        if selected_model:
            if selected_model != self.selected_model_path:
                self.selected_model_path = selected_model
                self.custom_model = YOLO(selected_model)
                results = self.custom_model(img_arrays)
        else:
    ...
    def correct_spelling_in_json(self, data):
        corrected = {}
        for key, value in data.items():
            if isinstance(value, str):
                corrected[key] = str(TextBlob(value).correct())
            else:
                corrected[key] = value
        return corrected
